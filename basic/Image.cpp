...
void Image::Binarization()
{
	...
	std::cout << "***********************************" << std::endl;
    while(ucThre != ucThre_new) { /* 找期望 、加权平均值*/
...
void Image::ConvAlgo()
{
    ...
    filter2D(m_Mat, m_Mat, -1 , kernel, anchor, 0, BORDER_DEFAULT );
}

void Image::FilterWave()
{
    boxFilter(m_Mat,m_Mat,-1,Size(85,20));
}
...
void Image::FloodFillDivide(std::vector<string> &pData,int nMin_area,char *pFilename,int flag)
{
	...
	while( ColorCount[k] )
	{
		//cout << "k=" << k << "ColorCount[k]=" << ColorCount[k] <<endl;
		if (ColorCount[k] > nMin_area)   /* 10 */
		{
			minX = minY = 100;
			maxX = maxY = -1;
	...
	{
		sprintf(window_name,"Character%d",i);
		//sprintf(buff,"%s%d.jpg",DIV_PATH,cnt++);
	        //vImage.at(i).SaveImg(buff);
		//cout << "this is here!!"  << __LINE__ << flag<<endl;
...
void Image::PrintPiexl()
{
	...
	for (i = 0; i < nWidth; ++i){	
		...
		piex = getPixel(i,j);
		if((piex==0)||(piex==255))
			cout << "is full 0 or 255" <<"  cnt=" << ++cnt << endl;
		else
			cout << "cnt1= " <<++cnt1 << "  piex = " << piex <<endl;
		}
    }
}
	...
	for (i = 0; i < nWidth; ++i){	
		...
		{ 
			if((i>0)&&(j>0)&&(i<nWidth-1)&&(j<nHeight-1)){
				leftup =getPixel(i,j);
				rightup = getPixel(i,j+1);
				leftdown= getPixel(i+1,j);	
				rightdown=getPixel(i+1,j+1);

			if((leftdown==rightup)&&(rightdown==leftup)&&(leftdown!=rightdown)){
